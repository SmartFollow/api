...
class UpdateAccessRuleGroupSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
	    ...
	    $administrators = Group::where('name', 'Administrateurs')->first();
	    if (!empty($administrators))
	    {
	    	$administrators->accessRules()->detach();
		    foreach ($rules['administrators'] as $rule)
			    $administrators->accessRules()->attach(AccessRule::where('route', $rule)->first());
	    }

	    $teachers = Group::where('name', 'Professeurs')->first();
	    if (!empty($teachers))
	    {
		    $teachers->accessRules()->detach();
		    foreach ($rules['teachers'] as $rule)
			    $teachers->accessRules()->attach(AccessRule::where('route', $rule)->first());
	    }

	    $employees = Group::where('name', 'Employés')->first();
	    if (!empty($employees))
	    {
		    $employees->accessRules()->detach();
		    foreach ($rules['employees'] as $rule)
			    $employees->accessRules()->attach(AccessRule::where('route', $rule)->first());
	    }

	    $students = Group::where('name', 'Étudiants')->first();
	    if (!empty($students))
	    {
		    $students->accessRules()->detach();
